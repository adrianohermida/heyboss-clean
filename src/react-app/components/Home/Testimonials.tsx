    ...
    <section className="py-24 bg-[var(--color-bg-alt)]" aria-labelledby="depoimentos-title">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 id="depoimentos-title" className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-4" style={{ color: mode === 'clear' ? 'var(--color-brand)' : 'var(--color-success)' }}>Depoimentos de Clientes sobre Defesa do Consumidor e Dívidas</h2>
          ...
          {[
            ...
            <div key={idx} className={`border shadow-xl p-8 rounded-3xl relative flex flex-col h-full ${mode === 'clear' ? 'bg-[var(--color-cardElevated)] border-[var(--color-border)]/30' : 'bg-[var(--color-card)] border-[var(--color-border)]/30'}`}>
              ...
              <div className="flex items-center gap-4 mt-auto">
                ...
                <div>
                  <p className="font-bold text-base" style={{ color: mode === 'clear' ? 'var(--color-brand)' : 'var(--color-success)' }}>{t.name}</p>
