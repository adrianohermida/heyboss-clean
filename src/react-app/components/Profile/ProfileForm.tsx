...
const ProfileForm = ({ profile, email, isCliente, saving, handleSave, message }: any) => {
  const [allConfigs, setAllConfigs] = React.useState<any>(null);
  React.useEffect(() => {
    import('../../../shared/form-configs.json').then(mod => setAllConfigs(mod.default || mod));
  }, []);

  if (!allConfigs) return <div>Carregando configurações...</div>;
